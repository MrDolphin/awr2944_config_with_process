...
APP_DIR="${APP_DIR:-/home/pi/camera_web_fusion}"
SERVICE_NAME="radar.service"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [[ "${EUID}" -ne 0 ]]; then
  echo "Run with sudo. Example: sudo APP_DIR=/home/pi/camera_web_fusion ./deploy/setup_rpi.sh"
  exit 1
fi
...
install -D -m 0644 "${SCRIPT_DIR}/radar-camera.env.example" "/etc/default/radar-camera"
