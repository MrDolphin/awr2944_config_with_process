...
现有 V0.1 代码产生的平铺文件可以保留，但新运行应按上述名字分组。整个
`results/` 运行目录（包括摘要）默认不提交 Git，避免每次运行污染状态。
需要成为论文或回归基线的小型摘要，必须经过人工复核后复制到该阶段受版本
控制的 `baselines/<baseline_id>/`；配置模板和结果字段契约也保存在阶段目录
而不是运行目录。重要大文件还必须备份到独立存储，不能只依赖 Git 忽略目录。
