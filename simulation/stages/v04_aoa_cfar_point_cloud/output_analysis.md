...
已将 `compRangeBiasAndRxChanPhase` 的 range bias、32 个实部/虚部值和 Tx-major 通道顺序解析为项目的 `[rx][tx]` 4×4 矩阵。当前 CFG 为单位复数序列，输出状态为 `cfg_identity_not_measured`，不代表已完成 TI 实测校准。结果见 `output/v04_19_cfg_calibration/`。

## V0.4.20 校准方向 A/B

已对同一组合成通道误差测试无补偿、直接因子、复数逆和共轭逆四种情况。结果见 `output/v04_20_calibration_direction/`；当前只证明数学接口方向，TI 固件约定仍须真实角反射器 A/B。
...
已加入 2D CA-CFAR、局部峰值筛选和多点 `(range, velocity, azimuth, elevation, power, x, y, z)` 输出。默认 Pfa=1e-3、训练单元 4×4、保护单元 1×1；结果仍标记 `cfar_peak_point_cloud_not_hardware_validated`。

## V0.4.24 海况点云统计

新增跨运行点云统计入口，统一输出点数、距离/速度/角度均值、标准差、P95 和最大功率。比较海况时必须固定采集时长、CFG、CFAR 参数、船体姿态和坐标系；统计结果不等同于检测概率或硬件角度精度。

## V0.4.25 海况点云图文报告

已增加每个海况的距离-速度图、方位-俯仰图、三维点云图和跨海况对比图，并生成 `sea_state_comparison.md`。图像解释必须结合点数、速度展宽、角度散布和 CFAR 参数，不能只凭颜色或点密度下结论。

## V0.4.26 海况点云自动判读

已增加相对第一行基准的描述性标记，默认阈值 20%，输出点数/速度/角度展宽变化。状态明确为 `descriptive_not_sea_state_classifier`，不会把仿真输出直接当成真实海况等级。

## V0.4.27 一键海况报告包

已将 V0.4.24 统计、V0.4.25 图文报告和 V0.4.26 描述性判读串成一个命令，按版本目录保存 `stats/`、`visual_report/`、`interpretation/` 和汇总 JSON，便于批次归档和领导汇报。
